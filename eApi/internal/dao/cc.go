...
func getCCConfig(cfgpath string) (*cccfg, error) {
	...
	if err = conf.GetConf(path, &cfg); err != nil {
		...
		log.Warnf("get cc config file succ, but ADDR IS EMPTY")
	} else {
		log.Infof("get cc config file succ, Addr: %v", cfg.Addr)
		return cfg, nil
	}
	// env
	if addr, exist := os.LookupEnv("REDIS_SVC_ADDR"); exist == false {
		...
		log.Warnf("get cc config env succ, but ADDR IS EMPTY")
	} else {
		log.Infof("get cc config env succ, Addr: %v", cfg.Addr)
		cfg.Addr = addr
		return cfg, nil
	}
