...
func getCCConfig(cfgpath string) (*cccfg, error) {
	...
	if err = conf.GetConf(path, &cfg); err != nil {
		...
		log.Printf("get cc config file succeed, but ADDR IS EMPTY")
	} else {
		log.Printf("get cc config file succeed, Addr: %v", cfg.Addr)
		return cfg, nil
	}
	// env
	if addr, exist := os.LookupEnv("REDIS_SVC_ADDR"); exist == false {
		...
		log.Printf("get cc config env succeed, but ADDR IS EMPTY")
	} else {
		log.Printf("get cc config env succeed, Addr: %v", cfg.Addr)
		cfg.Addr = addr
		return cfg, nil
	}
