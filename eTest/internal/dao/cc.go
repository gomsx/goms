...
func getCCConfig(cfgpath string) (*cccfg, error) {
	...
	if err = conf.GetConf(path, &cfg); err != nil {
		...
		log.Warn().Msgf("get db config file succ, but ADDR IS EMPTY")
	} else {
	...
	if addr, exist := os.LookupEnv("REDIS_SVC_ADDR"); exist == false {
		...
		log.Warn().Msgf("get db config env succ, but ADDR IS EMPTY")
	} else {
		log.Info().Msgf("get db config env succ, Addr: %v", cfg.Addr)
		cfg.Addr = addr
		return cfg, nil
	}
