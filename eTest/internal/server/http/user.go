...
func (s *Server) createUser(ctx *gin.Context) {
	...
	// 记录参数
	log.Ctx(c).Info().Msgf("start to create user, name:%v, sex:%v", name, sex)
...
func (s *Server) readUser(ctx *gin.Context) {
	...
	log.Ctx(c).Info().Msgf("start to read user, uid: %v", uid)
...
func (s *Server) updateUser(ctx *gin.Context) {
	...
	name := com.StrTo(ctx.PostForm("name")).String()
	sex := com.StrTo(ctx.PostForm("sex")).MustInt64()
	log.Ctx(c).Info().Msgf("start to update user, name: %v, sex %v", name, sex)
...
func (s *Server) deleteUser(ctx *gin.Context) {
	...
	c := ms.GetCtxVal(ctx)
	uid := com.StrTo(ctx.Param("uid")).MustInt64()
	log.Ctx(c).Info().Msgf("start to delete user, uid: %v", uid)
